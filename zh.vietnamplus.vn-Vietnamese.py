...
class VietnamPlus_Vietnamese(Spider.NewsSpider):

    def __init__(self):
        self.total_list = 10
        self.session = requests.Session()
        self.encoding = "utf-8"
        self.site = "VietnamPlus_Vietnamese"
        self.type = "news"
        self.initial_url = 'https://www.vietnamplus.vn/'
        self.news_list_url = 'https://www.vietnamplus.vn/{}/trang{}.vnp'
        self.now_detail_url = ""
        self.page_headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.181 Safari/537.36"
        }
        self.news_sections = ["chinhtri","thegioi","kinhte","xahoi","doisong","vanhoa","thethao","khoahoc","congnghe","moitruong","moitruong","dulich","tinthitruong",
        "chuyenla","rapnewsplus","newsgame"]
    ...
    def get_news_content(self,etree_html):#分析得到新闻正文
        texts = etree_html.xpath('//div[contains(@class, "article-body")]//p/text()')
                ...
                for page_number in range(1, total_page+1):
                    ...
                    if len(news_list) > 0:
                        for now_detail_url in news_list:
                            ...
                            if client.vietnam.VietnamPlus_Vietnamese.find({"url":self.now_detail_url}).explain()["executionStats"]["nReturned"]==0:
                                print("添加了",self.now_detail_url)
                                news_info = self.parse_news_details(self.now_detail_url)
                                if news_info is not None:
                                    client.vietnam.VietnamPlus_Vietnamese.insert_one(news_info)
                                    if count%50 == 0:
...
if __name__ == "__main__":
    ...
    scheduler.add_job(VietnamPlus_Vietnamese().run, 'interval', seconds=2)
    ...
    def my_listener(event):
        ...
        print(restart)
        if restart == 3:
            print("VietnamPlus_Vietnamese爬虫重启次数超过2次，停止！")
            os._exit(0)

        print("VietnamPlus_Vietnamese爬虫运行超时！重启！")
        scheduler.remove_all_jobs()
        scheduler.add_job(VietnamPlus_Vietnamese().run, 'interval', seconds=3600)
